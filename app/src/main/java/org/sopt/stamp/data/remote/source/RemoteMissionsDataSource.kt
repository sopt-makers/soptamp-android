...
internal class RemoteMissionsDataSource @Inject constructor(
    ...
    override suspend fun getAllMission(userId: Int): Result<List<MissionsData.MissionData>> {
        val result = kotlin.runCatching { soptampService.getAllMissions(userId).toData() }
        return when (val exception = result.exceptionOrNull()) {
            null -> result.map { it.missions }
            is UnknownHostException -> return Result.failure(ErrorData.NetworkUnavailable)
            else -> Result.failure(exception)
        }
    }

    override suspend fun getCompleteMission(userId: Int): Result<List<MissionsData.MissionData>> {
    ...
    override suspend fun getIncompleteMissions(userId: Int): Result<List<MissionsData.MissionData>> {
